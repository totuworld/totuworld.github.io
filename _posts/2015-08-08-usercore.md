...
`CreateUserUpgradeRow` 메서드를 Promise 체인 중 `usercore`테이블에 데이터를 생성하던 부분 아래에 위치시키면 되다.  
...
9 번째 줄 : 보시다시피 아이디가 등록되고 `then` 메서드를 통해  CreateUserUpgradeRow 메서드르 바로 호출되게 된다.
